...
namespace kanzi {
   ...
   public:
       ...
       static const short LITE_TYPE = 10; // TPAQLite (fork addition)
       static const short LITEXS_TYPE = 11; // TPAQLite-XS (fork addition, 2 ctxs)
   ...
   inline EntropyDecoder* EntropyDecoderFactory::newDecoder(InputBitStream& ibs, Context& ctx, short entropyType)
   {
       ...
       case LITE_TYPE:
           return new BinaryEntropyDecoder(ibs, new TPAQLitePredictor());

       case LITEXS_TYPE:
           return new BinaryEntropyDecoder(ibs, new TPAQLiteXSPredictor());
