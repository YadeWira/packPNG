...
namespace kanzi {
   ...
   public:
       ...
       static const short LITE_TYPE = 10; // TPAQLite (4 ctxs, no SSE/match) — fork addition
       static const short LITEXS_TYPE = 11; // TPAQLite-XS (2 ctxs, fastest BinaryEntropy) — fork addition
   ...
   inline EntropyEncoder* EntropyEncoderFactory::newEncoder(OutputBitStream& obs, Context& ctx, short entropyType)
   {
       ...
       case LITE_TYPE:
           return new BinaryEntropyEncoder(obs, new TPAQLitePredictor());

       case LITEXS_TYPE:
           return new BinaryEntropyEncoder(obs, new TPAQLiteXSPredictor());
